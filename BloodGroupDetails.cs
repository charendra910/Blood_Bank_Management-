...
{
    public partial class BloodGroupDetails : Form
    {
        ...
        private void txtBloodGroup_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(connectionstring))
                {
                    ...
                    string query = "SELECT * FROM AddNewDonor WHERE BloodGroup LIKE @searchText";
                    SqlCommand cmd = new SqlCommand(query, cn);

                    // Set the parameter value for the search text
                    cmd.Parameters.AddWithValue("@searchText", "%" + txtBloodGroup.Text + "%");
